...
# 이미지 삽입 — 반드시 빈 줄로 감싸고, 크기를 적절히 지정
doc.add_paragraph("")  # 이미지 앞 빈 줄 (필수 — 없으면 텍스트와 겹침)
doc.add_image("photo.png", width=42520, height=23918)  # A4 전체 너비 (16:9 비율)
doc.add_paragraph("")  # 이미지 뒤 빈 줄 (필수)
...
- **이미지와 텍스트 겹침** — rhwp가 textWrap 미지원, 이미지 높이만큼 텍스트를 밀어내지 못함
...
2. **시각 요소는 내용에 맞을 때만** — 표/목록/박스는 데이터나 비교가 있을 때 사용. 내용이 서술형이면 문단만으로 충분하다. 억지로 표를 만들지 않는다
...
A4 content width = **42520** HWPX 단위 (~150mm).

**기본값은 항상 전체 너비(42520)를 사용한다.** 로고/아이콘이 아닌 이상 반 너비를 쓰지 않는다.

이미지의 원본 비율을 유지하여 height를 계산한다:
```python
# 이미지 크기 계산 방법
from PIL import Image
img = Image.open("photo.png")
w, h = img.size
target_width = 42520  # A4 전체 너비 (기본값)
target_height = int(42520 * h / w)  # 비율 유지
doc.add_image("photo.png", width=target_width, height=target_height)
```

| 용도 | width | height 계산 |
|------|-------|------------|
| **사진/차트/다이어그램 (기본)** | **42520** | `42520 * 원본높이 / 원본너비` |
| 로고/아이콘만 | 8000~12000 | 비율 유지 |

**겹침 방지**: 이미지 앞뒤에 반드시 `add_paragraph("")` 삽입.
없으면 텍스트와 이미지가 겹쳐서 보임.
